...
BGCODE_CONVERT_EXPORT EResult from_ascii_to_binary(FILE& src_file, FILE& dst_file, const BinarizerConfig& config)
{
    ...
    static constexpr const std::string_view GeneratedByPrusaSlicer = "generated by OrcaSlicer"sv;
    ...
    if (!parser.parse([&](GCodeReader& r, const GCodeReader::GCodeLine& line) {
        ...
        if (!reading_config) {
            if (sv_line.find("CONFIG_BLOCK_START") != std::string_view::npos) {
                reading_config = true;
                processed_lines.emplace_back(lines_counter++);
                return;
            }
            ...
            if (sv_line.find("CONFIG_BLOCK_END") != std::string_view::npos) {
                ...
                if (pos == std::string_view::npos) {
                    ...
                    std::cout << "Unable to find key/value pair" << std::endl;
                    return;
                }
        ...
        if (!reading_thumbnail.has_value()) {
            ...
            if (reading_thumbnail.has_value()) {
                ...
                if (pos == std::string_view::npos) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Unable to find thumbnail string" << std::endl;
                    return;
                }
                ...
                if (rect.first == 0 || rect.second == 0) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Unable to get thumbnail size" << std::endl;
                    return;
                }
                ...
                if (data_size == 0) {
                    ...
                    std::cout << "Unable to get thumbnail data" << std::endl;
                    return;
                }
            ...
            if (sv_line.find(ThumbnailPNGEnd) == 0) {
                if (*reading_thumbnail != EThumbnailFormat::PNG) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Invalid thumbnail format (PNG)" << std::endl;
                    return;
                }
                thumbnail_end = true;
            }
                ...
                if (*reading_thumbnail != EThumbnailFormat::JPG) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Invalid thumbnail format (JPG)" << std::endl;
                    return;
                }
                thumbnail_end = true;
            }
                ...
                if (*reading_thumbnail != EThumbnailFormat::QOI) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Invalid thumbnail format (QOI)" << std::endl;
                    return;
                }
                thumbnail_end = true;
            }
            ...
            if (thumbnail_end) {
                ...
                if (curr_thumbnail_data_loaded != curr_thumbnail_data_size) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Thumbnail data mismatch" << std::endl;
                ...
                if (curr_thumbnail_data_loaded + sv_line.size() > curr_thumbnail_data_size) {
                    parse_res = EResult::InvalidAsciiGCodeFile;
                    std::cout << "Thumbnail data overflow" << std::endl;
                    return;
                }
                ThumbnailBlock& thumbnail = binary_data.thumbnails.back();
    ...
    if (reading_config) {
        std::cout << "Still reading config" << std::endl;
        return EResult::InvalidAsciiGCodeFile;
    }

    if (reading_thumbnail.has_value()) {
        std::cout << "Data still remains for thumbnail reading" << std::endl;
        return EResult::InvalidAsciiGCodeFile;
    }
    
    if (!producer_found) {
        std::cout << "Unable to find producer" << std::endl;
        return EResult::InvalidAsciiGCodeFile;
    }
